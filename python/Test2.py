...
def crawler():
    ...
    for status in tweepy.Cursor(api.user_timeline, screen_name=name, tweet_mode="extended").items():
        ...
        for i in messages:
            ...
            score = [s for s in proc if ('บาดเจ็บ' in s) or (
                'ตาย' in s) or ('เสียชีวิต' in s) or ('สูญหาย' in s) or ('สูญเสีย' in s) or ('เหยื่อ' in s) or ('แขนขาด' in s) or ('ขาขาด' in s) or ('อดอยาก' in s) ]
    ...
    while True:
        if(datanoti == noti and timete == time):
                countduplicate+=1;
        elif(datanoti != noti and timete != time):
            disaster1 = disaster2[0]
            score1 = score2[0]
            province1 = province2[0]
            latitude = lati(province1)
            longtitude = longti(province1)
            col.insert_one({"disaster":disaster1,"province":province1,"score":score1,"longtitude":longtitude,"latitude":latitude,"retweet":retweet,"like":like,"noti":noti,"time":time})
            print(score1)
            print(disaster1)
            print(province1)
            print(longtitude)
